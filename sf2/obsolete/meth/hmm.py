...
def hmmSample():
    ...
    model.startprob_ = np.array([0.1, 0.5, 0.4])
    model.transmat_ = np.array([[0.0, 1.0, 0.0],
                                [0.1, 0.5, 0.4],
                                [0.2, 0.6, 0.2]])
...
def ryuMove(winid, smack, pos):
    ...
    _, Z = hmmSample()
    #pre smack-learning, attack(freq) 0:smack updateMat, def()
    #down,L-R (spped(rate)) [seqLearning]
